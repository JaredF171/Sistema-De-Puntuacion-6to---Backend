...
class ApiIntegrationTests(APITestCase):
    ...
    def test_ai_feedback_endpoint_returns_feedback(self):
        payload = {
            "puntaje": 17.5,
            "kpis": {"puntualidad": 8.5, "colaboracion": 9.0},
            "comentario": "Buen desempeño pero debe documentar más",
        }
        res = self.client.post('/api/ai/feedback/', payload, format='json')
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.data.get('status'), 'success')
        feedback = res.data['data']['feedback']
        self.assertIn("Evaluación general", feedback)
